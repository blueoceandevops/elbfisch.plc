...
* VERSION   : $Revision$
* DATE      : $Date$
* PURPOSE   : implements the S7 data item UDT5$0
...
public class UDT5$0 extends LobRxTx{

    private IntRxTx 		field1;
    private IntRxTx 		field2;
    private UDT5$0$0 		nStruct;
    private IntRxTx 		field3;
    private BitRxTx 		field4;

    //constructor for standalone use
    public UDT5$0(Connection conn, Address address) throws IndexOutOfRangeException{
        super(conn, address, 0, null);
        setData(conn.generateDataObject(getSize()));
    }

    //constructor for use inside a structure
    public UDT5$0(Connection conn, Address address, int dataOffset, Data data) throws IndexOutOfRangeException{
    ...
    @SuppressWarnings("empty-statement")
    public UDT5$0$0 getNStruct() throws AddressException, IndexOutOfRangeException{
        if (nStruct == null){
            int byteOffset = getAddress().getByteIndex() + 4;
            Address addr = null;
            try{addr = (Address)getAddress().clone();}catch(CloneNotSupportedException exc){};
            addr.setByteIndex(byteOffset);
            addr.setSize(UDT5$0$0.getSize());
            nStruct = new UDT5$0$0(getConnection(), addr, byteOffset, getData());
        }
        return nStruct;
    }
    ...
    public IntRxTx getField3() throws AddressException, IndexOutOfRangeException{
        if (field3 == null){
            int byteOffset = getAddress().getByteIndex() + 12;
            Address addr = null;
            try{addr = (Address)getAddress().clone();}catch(CloneNotSupportedException exc){};
            addr.setByteIndex(byteOffset);
            addr.setSize(IntRxTx.getSize());
