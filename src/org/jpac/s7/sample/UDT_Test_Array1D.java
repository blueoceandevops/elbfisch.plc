...
* MODULE    : UDT_Test_Array1D.java 
* VERSION   : $Revision$
* DATE      : $Date$
* PURPOSE   : implements the S7 data item UDT_Test_Array1D
...
public class UDT_Test_Array1D extends LobRxTx{

    private BitRxTx[] 		Feld1D;

    //constructor for standalone use
    public UDT_Test_Array1D(Connection conn, Address address) throws IndexOutOfRangeException{
        super(conn, address, 0, null);
        setData(conn.generateDataObject(getSize()));
    }

    //constructor for use inside a structure
    public UDT_Test_Array1D(Connection conn, Address address, int dataOffset, Data data) throws IndexOutOfRangeException{
    ...
    public BitRxTx getFeld1D(int index1) throws AddressException, IndexOutOfRangeException{

        assertIndexRange(1, 1, 21, index1);

        if (Feld1D == null){
            Feld1D = new BitRxTx[21];
        }
        //check, if the item already has been accessed
        if (Feld1D[index1 - 1] == null){
            //if not, instantiate a new representation
            //and store it for subsequent uses
            int byteOffset = getAddress().getByteIndex() + 0 + (index1 - 1) / 8 ;
            Address addr = null;
            ...
            Feld1D[index1 - 1] = new BitRxTx(getConnection(), addr, byteOffset, getData());
        }
        return Feld1D[index1 - 1];
    }
}
